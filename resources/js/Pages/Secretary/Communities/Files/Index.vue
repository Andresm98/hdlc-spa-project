<template>
  <div class="w-full shadow sm:rounded-md py-5">
    ...
    >
      Almacenamiento de archivos de la Comunidad
    </h6>
    ...
    <form @submit.prevent="submit" class="p-2">
      <div class="px-4 py-5 bg-white sm:p-6">
        <div>
          <small
            >En el presente apartado usted puede almacenar archivos privados de cada cada
            comunidad u obra, por ejemplo puede guardar PDFs que contengan información
            relacionada a los trámites que se realizan en cada comunidad.
            <br />
          ...
          >
            <div class="space-y-1 text-center">
              ...
              <p class="text-xs text-gray-500">PDF, DOCX, PNG, JPG, GIF max 10MB.</p>
    ...
    <div class="px-4 py-5 m-4 bg-gray-200 sm:p-6 rounded-lg">
      <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8 p-4 overflow-y-auto h-80">
        <div v-if="this.allFiles != null" class="px-5">
          ...
          <br />
          <small v-if="this.allFiles.length == 100" class="text-red-600">
            No dispone de espacio para almacenar más archivos, en su carpeta personal sólo
            puede almacenar un máximo de 100 archivos.
          </small>
        </div>
        <div
          v-if="this.allFiles"
...
export default {
  ...
  mounted() {
    axios
      .get(
        this.route("secretary.communities.files.index", {
  ...
  methods: {
    ...
    confirmationFileUpdate(file) {
      ...
      axios
        .get(
          this.route("secretary.communities.files.edit", {
            file_id: file.id,
          })
        )
        .then((response) => {
    ...
    deleteFile() {
      this.deleteFileForm.delete(
        this.route("secretary.communities.files.delete", {
