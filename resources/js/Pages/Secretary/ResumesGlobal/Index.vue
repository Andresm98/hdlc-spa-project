<template>
  <app-layout>
    ...
    >
      ...
      <div class="mt-2">
        <p>
          ...
          resumenes anuales de todas las comunidades u obras de la compañía.
        </p>
      </div>
                      ...
                      <template #dropdown>
                        ...
                        >
                          <a
                            ...
                            :href="
                              route(
                                'secretary.resumes.communities.pdf',
                                this.params
                              )
                            "
                            >PDF</a
                          ...
                          <a
                            ...
                            :href="
                              route(
                                'secretary.resumes.communities.export.excel',
                                this.params
                              )
                            "
                            >Excel</a
    ...
    >
      <template #title> Datos del Nuevo Resumen</template>
          ...
          <div class="w-full lg:w-12/12 px-4 mb-2">
            <div class="relative w-full">
              ...
              <div :class="{ invalid: isInvalidCommunity }">
                <multiselect
                  :searchable="true"
                  v-model="selectCommunity.selectedCommunity"
                  :options="selectCommunity.options"
                  :close-on-select="true"
                  :clear-on-select="false"
                  :custom-label="customLabel"
                  @select="onselectedCommunity"
                  @search-change="onSearchCommunityChange"
                  track-by="comm_name"
      ...
      <template #footer>
        ...
        <jet-button class="ml-3" @click="createResume" v-if="navigationOp == 1">
          Crear
        </jet-button>
    ...
    >
      <template #title>
        Datos del Registro del Resumen Anual
        <span
  ...
  watch: {
    ...
    params: {
      handler: throttle(function () {
        ...
        this.$inertia.get(this.route("secretary.resumes.index"), params, {
          replace: true,
          preserveState: true,
          preserveScroll: true,
          onSuccess: () => {},
  ...
  methods: {
    ...
    onSearchCommunityChange(search) {
      ...
      if (search != null) {
        axios
          .get(
            this.route("secretary.resumes.communities.index", {
              search: search,
            })
          )
          .then((response) => {
    ...
    submitUpdate() {
      Inertia.put(
        route("secretary.communities.activity.update", {
          resume_id: this.resumeBeingUpdated.id,
    ...
    updateTable() {
      axios
        .get(
          this.route("secretary.communities.activity.resume.index", {
            resume_id: this.resumeBeingUpdated.id,
          })
        )
        .then((res) => {
    ...
    updateTableStaff() {
      axios
        .get(
          this.route("secretary.communities.staff.resume.index", {
            resume_id: this.resumeBeingUpdated.id,
            option: 2,
          })
        )
    ...
    submitUpdateStaff() {
      Inertia.put(
        route("secretary.communities.staff.resume.update", {
    ...
    refreshStaff(staff) {
      console.log(staff);
      Inertia.put(
        route("secretary.communities.staff.resume.refreshlist", {}),
        staff,
        {
          preserveScroll: true,
          preserveState: true,
