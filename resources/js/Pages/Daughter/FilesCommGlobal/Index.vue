<template>
  <app-layout>
                ...
                <tr v-for="file in this.files_list.data" :key="file">
                  ...
                  >
                    <div class="mx-auto flex gap-10">
                      ...
                      <a
                        :href="
                          this.route('daughter.communities.files.show', {
                            file_id: file.id,
                          })
                        "
                      >
    ...
    >
      <template #title>
        Datos del Nuevo Archivo asignado a: {{ community.comm_name }}
      </template>
...
export default {
  ...
  watch: {
    params: {
      handler: throttle(function () {
        ...
        this.$inertia.get(
          this.route("daughter.filesglobal.communities.index"),
          params,
          {
            replace: true,
            preserveState: true,
  ...
  methods: {
    ...
    confirmationFileUpdate(file) {
      ...
      axios
        .get(
          this.route("daughter.communities.files.edit", {
            file_id: file.id,
          })
        )
        .then((response) => {
    ...
    onSearchCommunityChange(search) {
      ...
      if (search != null) {
        axios
          .get(
            this.route("daughter.filesglobal.communities.search", {
              search: search,
            })
          )
          .then((response) => {
